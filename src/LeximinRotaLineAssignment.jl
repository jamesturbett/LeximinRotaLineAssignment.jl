...
# Arguments
- `preferences`: An \$R \\times L\$ matrix where row `r` lists rota lines (`1:L`) 
...
2. **Equitable Balancing:** Line loads are strictly bounded between `⌊R/L⌋` and `⌈R/L⌉`.
...
# Returns
- `Vector{Int}`: Assigned rota line ID (`1:L`) for each resident (`1:R`).
"""
function assign_residents_to_lines(preferences, rng = Random.default_rng())
    R, L = size(preferences)
    all(isperm, eachrow(preferences)) || throw(ArgumentError("Each row must be a permutation of line IDs 1:$L."))
    ...
    @variable(model, x[1:R, 1:L], Bin)

    # Each resident is assigned to exactly one rota line
    @constraint(model, [r in 1:R], sum(x[r, l] for l in 1:L) == 1)

    # Balanced capacities across all lines (bounded between ⌊R/L⌋ and ⌈R/L⌉)
    @constraint(model, [l in 1:L], div(R, L) <= sum(x[r, l] for r in 1:R) <= cld(R, L))

    # Leximin optimization: sequentially minimize resident counts at worst choice (L) down to 2nd choice
    for k in L:-1:2
        count_k = sum(x[r, preferences[r, k]] for r in 1:R)
    ...
    @objective(model, Min, sum(rand(rng) * x[r, l] for r in 1:R, l in 1:L))
    optimize!(model)

    # Extract the assigned line ID (1:L) for each resident (1:R)
    return [argmax(l -> value(x[r, l]), 1:L) for r in 1:R]
end
